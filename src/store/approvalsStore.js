  ...
  actions: {
    ...
    revokePermissions(
      ...
      try {
        ...
        if (state.searchedWallet !== rootState.walletStore.connectedWallet)
          throw "The connected Wallet and the Wallet you want to revoke permissions for are not the same.";
        ...
        contract
          .approve(spender, 0, { gasLimit: 500000 })
          .then(async (t) => {
            await provider.waitForTransaction(t.hash);
            commit(
              "generalStore/ADD_ALERT",
              "Permissions revoked successfully.  Give it some time to reflect in the UI.",
              { root: true }
            );
            commit("SET_LOADING", false);
